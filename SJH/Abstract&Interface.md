...
- 클래스 구현부 내부에 ***<u>추상 메소드</u>*** **가 하나이상 포함되거나 abstract로 정의된 클래스**

  - 이 말은 일반 메서드의 구현도 가능하다는 뜻이다.
...
- 추상 클래스에서 정의된 추상적인 기능은 하위 클래스에서 상세 구현한다.
  ...
  미완성으로 남겨놓은 이유는 상속받는 클래스에 따라 메서드의 내용이 달라질 수 있기 때문다.
  ~~~
...
- #### 인터페이스 장점
  ...
  **4. 독립적인 프로그래밍이 가능하다.**
  ...
  - ex_)
    ...
    - Interface I 는 실제 구현 내용을 감싸고 있는 껍데기이며, Class A는 껍데기 안에 어떤 알맹이(Class)가 있는지 몰라도 된다.
...
- **차이점**
  ...
  3. 추상클래스는 상속을 받아 기능을 확장 즉, 부모의 유전자를 물려받는다(IS-A). 인터페이스는 구현하는 모든 클래스에 대해 특정한 메서드가 반드시 존재하도록 강제하는 역할을한다.유전자가 아니라 사교적으로 필요에 따라 결합하는 관계라는 뜻이다(HAS-A).
...
| 상속구현  | class Sub **extends** super { <br>  메서드 재정의 // overriding <br>} | class Sub **implements** Interface1, Interface2 {   <br>  메서드 재정의 // overriding <br>} |
| 상속 특징 | 단일 상속                                                    | 다중 상속                                                    |
...
1.  **추상클래스란 무엇**인가?
   ...
   - 클래스 구현부 내부에 추상 메소드 가 하나이상 포함되거나 abstract로 정의된 클래스

   

2. **인터페이스란 무엇**인가?

   - 모든 메서드가 추상 메서드로 되어있는 추상클래스보다 더 추상화 된 개념

   

3. **인터페이스를 사용하는 이유**는 무엇인가?

   - 클래스의 선언과 구현을 분리할 수 있고,
   - 또한 인터페이스를 이용하여 간접적인 관계로 변경하면, 한 클래스의 변경이 관련된 다른 클래스에 영향을 미치지 않는 독립적인 프로그래밍이 가능하다.
