...
- **UDP 특징**
  ...
  >
>- 비연결형 서비스로 전송순서가 바뀔 수 있다.

  >**간단한 트랜잭션**
  >
>- setup과 종료, ACK가 필수불가결하여 복잡한 트랙잭션이 요구되는 TCP와 달리 UDP는 트랜잭션이 간단하다.
...
1. **전송 순서를 보장할 수 있는 프로토콜**은 무엇이며, 그 **이유**는 무엇인지 쓰시오.	
   - TCP, 패킷 전달을 가상 회선 방식을 쓰기 때문이다.
   - 데이터를 전송하기 전 논리적 연결을 설정하므로, 전송순서가 변하지 않는다.



2. TCP와 UDP가 사용되는 **계층**은 어디인가?
